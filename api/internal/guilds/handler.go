...
func (h *Handler) CreateInvite(w http.ResponseWriter, r *http.Request) {
	...
	if req.MaxAge != nil {
		if *req.MaxAge < 0 || *req.MaxAge > maxInviteAgeSec {
			errs.Write(w, errs.FormBody("Invalid Form Body: max_age must be 0-604800 seconds"))
			return
		}
		maxAge = *req.MaxAge
	}
	...
	if req.MaxUses != nil {
		if *req.MaxUses < 0 || *req.MaxUses > maxInviteUses {
			errs.Write(w, errs.FormBody("Invalid Form Body: max_uses must be 0-100"))
			return
		}
		maxUses = *req.MaxUses
	}
...
func (h *Handler) JoinInvite(w http.ResponseWriter, r *http.Request) {
	...
	if code == "" || len(code) > 32 {
		errs.Write(w, errs.FormBody("Invalid Form Body: bad invite code"))
		return
	}
