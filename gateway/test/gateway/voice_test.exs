defmodule Gateway.VoiceTest do
  ...
  describe "Voice signaling security" do
    test "routes signaling between peers in the same voice channel" do
      {:ok, _pid} = Actor.get_or_spawn(@test_guild_id)

      peer_a_session = "sess-peer-a"
      peer_b_session = "sess-peer-b"
      peer_a_uid = "user-a"
      peer_b_uid = "user-b"

      Cache.put_member_roles(peer_a_uid, @test_guild_id, [])
      Cache.put_member_roles(peer_b_uid, @test_guild_id, [])

      assert :ok == Actor.subscribe(@test_guild_id, peer_a_session, self(), peer_a_uid)
      ...
      peer_b_pid =
        spawn_link(fn ->
          loop = fn recur ->
            receive do
              {:dispatch, %{"type" => "VOICE_SIGNALING"} = event, _ts} ->
                send(parent, {:peer_b_received, event})
                recur.(recur)
      ...
      assert :ok == Actor.subscribe(@test_guild_id, peer_b_session, peer_b_pid, peer_b_uid)

      # Both join public voice channel
      assert {:ok, _} = Actor.update_voice_state(@test_guild_id, peer_a_uid, peer_a_session, %{"channel_id" => @public_voice_channel})
      assert {:ok, _} = Actor.update_voice_state(@test_guild_id, peer_b_uid, peer_b_session, %{"channel_id" => @public_voice_channel})

      # Peer A sends SDP Offer to Peer B
      signal_data = %{
        "channel_id" => @public_voice_channel,
        "to_user_id" => peer_b_uid,
        "type" => "offer",
        "payload" => %{"sdp" => "v=0...fake-sdp"}
      }

      assert :ok == Actor.voice_signaling(@test_guild_id, peer_a_uid, signal_data)

      # Peer B receives signaling envelope
      assert_receive {:peer_b_received, event}, 1000
      assert event["type"] == "VOICE_SIGNALING"
      assert event["payload"]["from_user_id"] == peer_a_uid
      assert event["payload"]["type"] == "offer"
      assert event["payload"]["payload"]["sdp"] == "v=0...fake-sdp"
    end

    test "rejects signaling if sender or receiver is not in the channel" do
      {:ok, _pid} = Actor.get_or_spawn(@test_guild_id)

      peer_a_session = "sess-peer-a-2"
      peer_b_session = "sess-peer-b-2"
      peer_a_uid = "user-a-2"
      peer_b_uid = "user-b-2"
      ...
      assert :ok == Actor.subscribe(@test_guild_id, peer_a_session, self(), peer_a_uid)
      assert :ok == Actor.subscribe(@test_guild_id, peer_b_session, self(), peer_b_uid)


      # Only Peer A joins voice channel
      assert {:ok, _} = Actor.update_voice_state(@test_guild_id, peer_a_uid, peer_a_session, %{"channel_id" => @public_voice_channel})
      ...
      # Rejected because target is not in the channel
      assert {:error, :target_not_in_channel} == Actor.voice_signaling(@test_guild_id, peer_a_uid, signal_data)

      # Rejected if sender is not in the channel
      assert {:error, :sender_not_in_channel} == Actor.voice_signaling(@test_guild_id, "intruder", signal_data)
    end
  end
